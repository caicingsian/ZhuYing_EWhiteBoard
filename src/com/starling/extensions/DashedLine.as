package starling.extensions
{
	 ...
	 * var myDashedDrawing:DashedLine = new DashedLine(this.graphics, 1, 8); // dotted line
	 * // var myDashedDrawing:DashedLine = new DashedLine(this.graphics, 5, 12); // dashed line
	 * // draw a square with dashed lines
	 * myDashedDrawing.moveTo(20, 20);
	 * myDashedDrawing.lineTo(300, 60);
	 * myDashedDrawing.curveTo(350, 200, 300, 300);
	 * myDashedDrawing.lineTo(50, 300);
	 * myDashedDrawing.lineTo(20, 20);
	 *
	 * @author Trevor McCauley, senocular.com
	 * @version 1.0.2
	 * @modified to ActionScript 3.0 by esaBear
	 */
	public class DashedLine {
		...
		public function curveTo(cx:Number, cy:Number, x:Number, y:Number):void {
			...
			var segLength:Number = this.curveLength(sx, sy, cx, cy, x, y, this._curveaccuracy);
			var t:Number = 0;
			var t2:Number = 0;
			var c:Array;
			if (this.overflow){
		...
		public function drawCircle( x:Number , y:Number , radius:Number ):void
		{
			var arcAngle:int = 4;
			moveTo( x , y );
			for( var i:int = 0 ; i < 360 / (arcAngle * 2) ; i++ )//360 / (arcAngle * 2)
			{
				drawArc( radius , i*(arcAngle*2) , arcAngle );
			}
		}
		
		public function drawArc( radius:Number , startAngle:Number , arcAngle:Number ):void
		{
		 ...
		 * the line. If a value isn??indicated, Flash uses 0x000000 (black).
		 * @param alpha An integer that indicates the alpha value of the line??color; valid values are
		 * 0?100. If a value isn??indicated, Flash uses 100 (solid). If the value is less than 0, Flash
		...
		private function curveSliceUpTo(sx:Number, sy:Number, cx:Number, cy:Number, ex:Number, ey:Number, t:Number=1):Array {
			if (t != 1) {
		...
		private function curveSliceFrom(sx:Number, sy:Number, cx:Number, cy:Number, ex:Number, ey:Number, t:Number=1):Array {
			if (t != 1) {
